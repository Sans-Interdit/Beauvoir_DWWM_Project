...
def test_determine_criterias():
    try:
        prompt = {"role": "user", "content": "je cherche un film avec un bateau de croisière qui s'échoue sur un iceberg"}
        result = json.loads(determine_criterias(prompt))
        expected_result = json.loads('{"format": "film", "key_words": ["bateau de croisière", "s\'échoue", "iceberg"]}')
